...
**Observability**
- **Langfuse** (`observability/tracing.py`) — LLM/agent tracing. `get_handler()` wires every `agents.graph.run()` call via LangChain's callback mechanism (`langfuse.langchain.CallbackHandler`); `wrap_completion()` wraps `agents/llm_router.py`'s raw `litellm.completion()` calls with a manual Langfuse generation span, applied at import time in `llm_router.py` whenever keys are set. That second path exists because litellm's own built-in `success_callback=["langfuse"]` hook is broken against the current Langfuse v4 SDK (`AttributeError: module 'langfuse' has no attribute 'version'` — litellm's integration code expects a `langfuse.version` submodule that v4 doesn't expose; confirmed still broken on litellm 1.101.0, the latest as of this writing), and litellm's own error-swallowing meant it wasn't obviously broken — every `/ask` question was silently falling back to the old stuffed-prompt behavior instead of raising. Manual instrumentation via `langfuse.get_client().start_as_current_observation()` sidesteps that code path entirely and isn't tied to any particular litellm version. Both mechanisms auto-detect `LANGFUSE_PUBLIC_KEY`/`SECRET_KEY` and no-op otherwise.
